...
const Profile = (props) => {
    ...
    const handleSubmit= async(e)=>{
        e.preventDefault();
        let res = await axios.put(`https://socalapp-backend.onrender.com/users/update/${userDetails._id}`,details);
        let data = res.data
        console.log(data)
        if(data.success){
    ...
    const handleCoverChanger=(e)=>{
      ...
      let reader = new FileReader();
        ...
        reader.onload=async()=>{
          ...
          let res = await axios.put(`https://socalapp-backend.onrender.com/users/update/${userDetails._id}`,{coverPic:reader.result});
          let data = res.data
          console.log(data)
          if(data.success){
            props.getUserDetails()
  ...
  let file = e.target.files[0];
      ...
      let reader = new FileReader();
        ...
        reader.onload=async()=>{
          ...
          let res = await axios.put(`https://socalapp-backend.onrender.com/users/update/${userDetails._id}`,{profilePic:reader.result});
          let data = res.data
          console.log(data)
          if(data.success){
            props.getUserDetails()
...
<div className=" mx-auto w-[90%] bg-white shadow-xl rounded-lg text-gray-900">
  ...
  <div className='absolute right-0 top-1 cursor-pointer'>
    ...
    <img className="object-cover object-center h-32 w-[100%] rounded-full" src={userPics.profilePic? userPics.profilePic : userDetails.profilePic} alt="Woman looking front" />
  </div>
